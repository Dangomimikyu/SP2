...
void SceneMG3::Update(double dt)
{
...
void SceneMG3::Render()
{
	...
	// world text
	modelStack.PushMatrix();
	RenderText(meshList[GEO_TEXT], "THIS IS MG3", Color(0, 1, 0));
	modelStack.PopMatrix();

	//on-screen text
	RenderTextOnScreen(meshList[GEO_TEXT], "Current FPS: " + print_fps(), Color(1, 0, 1), 3, 0, 0);
...
void SceneMG3::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void SceneMG3::RenderMesh(Mesh* mesh, transform object, bool enableLight)
{
...
void SceneMG3::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneMG3::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
